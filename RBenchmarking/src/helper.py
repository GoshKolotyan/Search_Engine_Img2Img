...
class DifferenceAnalyzer:
    def __init__(self, path_relevant: str, path_not_relevant: str, save_path: str):
    ...
    def visualize_difference(self) -> None:
        ...
        if self.diff_df.empty:
            raise ValueError(
                "Difference DataFrame is empty. Run compute_difference() first."
            )

        plt.figure(figsize=(20, 12))
        plt.bar(
            self.diff_df["Model Name"][::-1], self.diff_df["Difference Score (%)"][::-1]
        )
    ...
    def __call__(
        self, visualize: bool = False, threshold_percent: float = 10.0
    ) -> pd.DataFrame:
...
if __name__ == "__main__":
    ...
    analyzer = DifferenceAnalyzer(
        path_relevant=path_1, path_not_relevant=path_2, save_path="../OUTPUTS/Analysis"
    )
