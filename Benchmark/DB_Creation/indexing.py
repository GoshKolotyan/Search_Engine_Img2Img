...
class ModelDatabaseBuilder:
    ...
    def _computer_features_swin(self, image: Image.Image) -> np.ndarray:
        if image is None:
            return None
    ...
    def _compute_features_dino(self, image: Image.Image) -> np.ndarray:
        if image is None:
            return None
            ...
            if os.path.isdir(category_path):
                for filename in tqdm(
                    ...
                    if filename.lower().endswith((".png", ".jpg", ".jpeg")):
                        ...
                        original_features = self._computer_features_swin(image) if"swin" in self.model_name else self._compute_features_dino(image)
                        ...
                        for aug_suffix, aug_image in augmented_versions:
                            ...
                            aug_features = self._computer_features_swin(aug_image) if "swin" in self.model_name else self._compute_features_dino(aug_image)
